        ...
        for I = TemplatesToTest(1) : TemplatesToTest( TemplatesToTestElements )
            ...
            if templatesPresent(templateCurrentlyTesting) > 0 && numel(find(SelectSpecificTemplatesToAvoidUsing == templateCurrentlyTesting)) == 0
                ...
                if firstRun == 1 %Slow first time
                    ...
                    KFSignal = imfilter(Oldsignal,H,'replicate');
                    KFTemplate = imfilter(template,H,'replicate');

                    %L1 = L1(:,ChannelsToInvestigate);
                    time = toc;
                ...
                elseif strcmp(AlgorithmToTest, 'LaplacianV2') == 1
                    result = TemplateLaplacianV2(KFSignal, KFTemplate, ViewFiguresRunning, ShowProgressBar, ShowFunctionExcTime,pathToNormXcorr2General, ChannelsToInvestigate);
                    threshold = repmat(0.7,1,MaximumNumberOfTemplates);
                elseif strcmp(AlgorithmToTest, 'LaplacianDrift') == 1    
                    if HandleDriftChannel > 0
                        result = TemplateLaplacianWithDrift(KFSignal, KFTemplate, ViewFiguresRunning, ShowProgressBar, ShowFunctionExcTime,pathToNormXcorr2General, HandleDriftChannel, ChannelsToInvestigate);
                    else  
                        result = TemplateLaplacianV2(KFSignal, KFTemplate, ViewFiguresRunning, ShowProgressBar, ShowFunctionExcTime,pathToNormXcorr2General, ChannelsToInvestigate);
                    end
                    threshold = repmat(0.7,1,MaximumNumberOfTemplates);
                ...
                if strcmp(UsePreFoundTemplateThreshold, 'YES') == 1
                    ...
                    if strcmp(AlgorithmToTest, 'LaplacianDrift') == 1 
                        result = max(result, [], 2);
                    end
