...
for X = 1: numel(NumberOfTemplateSamplesUsed)
    ...
    CorrelationResultMatrix = single(zeros(((signalLength_s*fs)-templateSizeTestingThisRound), MaximumNumberOfTemplates));
    ...
    for Y = 1 : numel(ChannelsToTest)
        ...
        for I = TemplatesToTest(1) : TemplatesToTest( TemplatesToTestElements )
            ...
            template1 = PrepareTemplate( strcat(DiectoryToEvaluate,'\templates.npy'), templateCurrentlyTesting, [1:32], ...
                                                                             templateGain, pathToNPYMaster, ViewFiguresRunning, ShowFunctionExcTime);
            
            template = single(template1);                                                             
            %% Save orignal Template
            if strcmp(saveFilteredData, 'YES')
                
                SaveData(template, strcat(PathToFilteredData, '\Templates'), strcat('template_', num2str(I), '.bin'));
            end
            ...
            if templatesPresent(templateCurrentlyTesting) > 0 && numel(find(SelectSpecificTemplatesToAvoidUsing == templateCurrentlyTesting)) == 0
                ...
                if firstRun == 1 %Slow first time
                    Oldsignal1 = PrepareData( strcat(DiectoryToEvaluate,'\sim_binary.dat'), 1:32, rez, signalOffset, ...
                                                                                 signalLength_s, signalGain, filter, filterType, fs, ViewFiguresRunning, ShowFunctionExcTime);
                    
                     
                    H = fspecial('laplacian'); % Original
                    Oldsignal = imfilter(Oldsignal1,H,'replicate');
                ...
                %% Save FilteredData ??
                if strcmp(saveFilteredData, 'YES')
                    %SaveRezFile( PathToFilteredData, 'RezInfo.bin', rez )
                    
                    %SaveData(signal, PathToFilteredData, 'channelFilteredData.bin');
                    
                    %SaveFilteredData( signal, PathToFilteredData);
                    
                    %SaveFilteredTemplate ( template, PathToFilteredData, templateCurrentlyTesting);
                end
                ...
                elseif strcmp(AlgorithmToTest, 'NSAD') == 1
                    result = TemplateNSAD( signal, template, ViewFiguresRunning, ShowProgressBar, ShowFunctionExcTime);
                    threshold = repmat(0.5,1,MaximumNumberOfTemplates);
                ...
                elseif strcmp(AlgorithmToTest, 'Laplacian') == 1
                    ...
                    threshold = repmat(0.5,1,MaximumNumberOfTemplates);
                elseif strcmp(AlgorithmToTest, 'LaplacianSSD') == 1
                    result = TemplateLaplacianNSSD(signal, template, ViewFiguresRunning, ShowProgressBar, ShowFunctionExcTime,pathToNormXcorr2General);
                    threshold = repmat(0.7,1,MaximumNumberOfTemplates);
                elseif strcmp(AlgorithmToTest, 'SobelNXCOR') == 1
                    result = TemplateSobelNXCOR_2D(signal, template, ViewFiguresRunning, ShowProgressBar, ShowFunctionExcTime,pathToNormXcorr2General);
                    threshold = repmat(0.7,1,MaximumNumberOfTemplates);                 
                elseif strcmp(AlgorithmToTest, 'OtherFeature') == 1
                ...
                tic 
                [finalResultPeaks, finalResultTimes] = GetValuesAboveThreshold(result, single(threshold(I)), IsAlgorithmSimilarityBased);    
                time = toc;
                fprintf('Prediction algorithm time: %.2f\n',time);
                ...
                [precision, recall, fallout, TP, TN, FP, FN] = GenerateConfusionMatrixWithSlack(finalResultTimes, rez_st3_templateRelevant,2, totalSamples);
        ...
        stdSampleSize = CalculateSampleSizeStd(TruthTemplateArray);
        SampleSize = CalculateSampleSize(ConfLevel,ErrorMargin,stdSampleSize);
        CalculateStatisticalSampleSize(TruthTemplateArray, ViewFiguresRunning);
